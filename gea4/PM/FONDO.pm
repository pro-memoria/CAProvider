...
sub _listrecords_ead {
	...
	my $sql = q[
	SELECT RecordID AS object_id, RecordID as idno, Denominazione as name, '1' as is_preferred, 'fondo' as item_value,
	Contenuto as scopecontent,
...
=test		
		if ($dataDA eq ''){
		    $attrs->{'data_range'} = "FINO AL $dataA";
		} elsif ($dataA eq '') {
		    $attrs->{'data_range'} = "DAL $dataDA";
		} else {		
