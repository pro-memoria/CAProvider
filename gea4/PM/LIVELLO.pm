...
sub _listrecords_ead {
	...
	my $sql = q[
	SELECT RecordID AS object_id, RecordID as idno, Denominazione as name, '1' as is_preferred, livello as item_value, parentid parent_uuid,
...
sub _perl_ead {
    ...
    my $struct = Promemoria::SAN::EAD::perl_ead(
		...
		value => encode_entities($obj->{name}),
	    },
	);
    return $struct;
}
