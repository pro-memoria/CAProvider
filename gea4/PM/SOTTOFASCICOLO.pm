...
sub _listrecords_ead {
	...
	my $sql = q[
	SELECT RecordID AS object_id, RecordID as idno, Titolo as name, '1' as is_preferred, 'sottofascicolo' as item_value, parentid parent_uuid,
	Contenuto as scopecontent, sottofascicolo_nr numero,
	data_da_giorno, data_da_mese, data_da_anno,
	data_a_giorno, data_a_mese, data_a_anno
	FROM sottofascicolo
	ORDER BY sottofascicolo_nr
	];
...
#    for my $object_id (sort {$a <=> $b} keys %objects) {
	 for my $i (0 .. $#ordered_object_ids) {
		...
		$attrs->{'scopecontent'} = encode_entities($objects{$object_id}{'scopecontent'});
